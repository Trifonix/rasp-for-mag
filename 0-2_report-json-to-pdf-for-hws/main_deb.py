...
# --- 1. Регистрация шрифтов для LINUX ---
# Устанавливаем пути к шрифтам DejaVu (есть в Debian по умолчанию)
...
if not os.path.exists(font_regular):
    print(f"Шрифт {font_regular} не найден!")
    print("Установите: sudo apt install fonts-dejavu-core")
    exit(1)

if not os.path.exists(font_bold):
    print(f"Шрифт {font_bold} не найден!")
    print("Установите: sudo apt install fonts-dejavu-core")
    exit(1)
...
with open("./0-2_report-json-to-pdf-for-hws/title.json", "r", encoding="utf-8") as f:
    title_data = json.load(f)

with open("./0-2_report-json-to-pdf-for-hws/report.json", "r", encoding="utf-8") as f:
...
# --- 4. Стили (Изменили имя шрифта) ---
styles = {
    'Info': ParagraphStyle('Info', fontName='DejaVuSerif', fontSize=14, alignment=TA_CENTER, leading=14),
    'Subject': ParagraphStyle('Subject', fontName='DejaVuSerif-Bold', fontSize=14, alignment=TA_CENTER, leading=16, spaceBefore=16),
    'Title': ParagraphStyle('Title', fontName='DejaVuSerif-Bold', fontSize=18, alignment=TA_CENTER, leading=22, spaceAfter=16),
    'Header': ParagraphStyle('Header', fontName='DejaVuSerif-Bold', fontSize=16, spaceBefore=18, spaceAfter=12, leading=18, keepWithNext=True),
    'SubHeader': ParagraphStyle('SubHeader', fontName='DejaVuSerif-Bold', fontSize=14, spaceBefore=12, spaceAfter=10, keepWithNext=True),
    'Normal': ParagraphStyle('Normal', fontName='DejaVuSerif', fontSize=14, alignment=TA_JUSTIFY, leading=16, spaceAfter=10),
    'RightAlignBlock': ParagraphStyle('RightAlignBlock', fontName='DejaVuSerif', fontSize=16, alignment=TA_LEFT, leading=14)
}
